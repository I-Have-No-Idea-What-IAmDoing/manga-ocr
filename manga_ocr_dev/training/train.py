...
def main(
    ...
    wandb.init(project="manga-ocr", name=run_name, config=config.app.model_dump())
    ...
    if config.app.training.torch_compile:
        model = torch.compile(model)

    # Create the training dataset with augmentations enabled
    train_dataset = MangaDataset(
        processor,
        dataset_config=config.app.dataset,
        max_target_length=config.app.model.max_len,
    )
    ...
    training_args = Seq2SeqTrainingArguments(
        ...
        run_name=run_name,
        **config.app.training.model_dump(by_alias=True),
    )
