...
class TestSyntheticDataGenerator(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        try:
            generator = SyntheticDataGenerator(background_dir=self.backgrounds_dir)
            self.assertIsNotNone(generator)
        except Exception as e:
            self.fail(f"SyntheticDataGenerator initialization failed with {e}")
    ...
    def test_process_random_text(self):
        ...
        generator = SyntheticDataGenerator(background_dir=self.backgrounds_dir)
        # Retry a few times in case of unlucky random generation (e.g., text too short)
        for _ in range(10):
            img, text_gt, params = generator.process(override_params={'color': '#000000'})
            if img is not None:
                break
        self.assertIsNotNone(img, "Failed to generate a valid image after multiple attempts.")
        self.assertIsInstance(img, np.ndarray)
        self.assertGreater(img.shape[0], 0)
        self.assertGreater(img.shape[1], 0)
    ...
    def test_font_size_control(self):
        ...
        generator = SyntheticDataGenerator(background_dir=None, min_font_size=40, max_font_size=50)
        _, _, params = generator.process("test")
    ...
    def test_target_size(self):
        ...
        target_size = (128, 128)
        generator = SyntheticDataGenerator(background_dir=self.backgrounds_dir, target_size=target_size)
    ...
    def test_min_output_size(self):
        ...
        min_size = 300
        generator = SyntheticDataGenerator(background_dir=self.backgrounds_dir, min_output_size=min_size)
    ...
    def test_legibility_check_discards_small_text(self, mock_rand):
        ...
        generator = SyntheticDataGenerator(background_dir=self.backgrounds_dir, min_font_size=5, max_font_size=6)
