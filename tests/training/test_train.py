"""Tests for the main training script.

This module contains integration tests for the `train.py` script, which
orchestrates the entire model training pipeline. The tests use extensive
mocking to verify that the main function correctly initializes all components
(config, model, datasets, trainer) and calls them in the expected sequence.
"""
...
class TestTrain(unittest.TestCase):
    """A test suite for the main training function."""
    ...
    def test_main_default_run(self, mock_load_config, mock_wandb, MockGetModel, mock_torch_compile, MockMangaDataset, MockMetrics, MockSeq2SeqTrainingArguments, MockSeq2SeqTrainer):
        """Tests the main training function with a default (None) run name.

        This test verifies that when no `run_name` is provided, the training
        pipeline correctly initializes, uses the `run_name` generated by
        `wandb`, and proceeds with the training process as expected.
        """
        # --- Setup Mocks ---
        # Mock config
    ...
    def test_main_with_custom_run_name_and_compile(self, mock_load_config, mock_wandb, MockGetModel, mock_torch_compile, MockMangaDataset, MockMetrics, MockSeq2SeqTrainingArguments, MockSeq2SeqTrainer):
        """Tests the main training function with a custom run name and `torch.compile` enabled."""
        # --- Setup Mocks ---
        # Mock config
        mock_config = MagicMock()
